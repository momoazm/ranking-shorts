...
### 21st.dev for added sections (REQUIRED on Moemen's site builds)
- For any non-canvas section (feature/bento grid, testimonials, logo cloud, pricing, refined CTA),
  pull a premium pattern from **21st.dev Magic** (`/ui` or the `@21st-dev/magic` MCP) first instead of
  inventing layout from scratch. Use `21st_magic_component_inspiration` to get usable design data.
- 21st.dev returns React/Tailwind. On vanilla scroll sites, take its **structure + interaction**
  (e.g. divided-grid hairlines, cursor-follow glow) and **reimplement in vanilla, restyled to the
  brand** — never ship its default colors/fonts. See `projects/website/web.md`.