...
- **momoclips (2026-07-04):** channel rebranding to @momoclips (YouTube + Instagram) + a new
  single-clip World-Cup format (`clip_autopost.py`: goals / viral moments (no iShowSpeed), posted one at a
  time), triggered by ~20-min polling. Go-live is GATED — see the header of
  `.github/workflows/worldcup_clips.yml`. The #5->#1 countdown (`rank_autopost.py`) still exists.
